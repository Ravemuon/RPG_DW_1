        ...
        <form method="GET" class="row g-3 align-items-end">
            ...
            <div class="col-md-2 d-flex gap-2">
                ...
                <a href="{{ route('sessoes.index', $campanha->id) }}"
                   class="btn btn-danger rounded-pill flex-fill"><i class="fas fa-times"></i></a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-xl-4 col-md-6">
            <div class="card bg-dark border-secondary rounded-4 shadow-lg h-100 p-3 transition-hover">
                ...
                <p class="text-muted small mb-4">{{ Str::limit($sessao->resumo ?? 'Sem resumo.', 120) }}</p>
                <div class="d-flex gap-2 pt-3 border-top border-secondary mt-auto">
                    <a href="{{ route('sessoes.show', [$campanha->id, $sessao->id]) }}"
                       class="btn btn-outline-info btn-sm rounded-pill flex-fill"><i class="fas fa-eye"></i> Detalhes</a>
                    @can('update', $campanha)
                    <a href="{{ route('sessoes.edit', [$campanha->id, $sessao->id]) }}"
                       class="btn btn-outline-warning btn-sm rounded-pill flex-fill"><i class="fas fa-edit"></i> Editar</a>
                    @endcan
                    @if($sessao->status == 'concluida')
                    <a href="{{ route('sessoes.exportarPdf', [$campanha->id, $sessao->id]) }}"
                       class="btn btn-outline-primary btn-sm rounded-pill flex-fill"><i class="fas fa-file-pdf"></i> Relatório</a>
                    @endif
                    @can('delete', $campanha)
...
<style>
    ...
    .transition-hover:hover { transform: translateY(-4px); box-shadow:0 10px 15px rgba(0,0,0,.5);}
